...
public void openNewwindowImage()
{
    ...
    public void openNewWindowFin (){


        try {
            FXMLLoader fxmlLoader = new FXMLLoader(HelloController.class.getResource("Fin.fxml"));
            Parent root1 = (Parent) fxmlLoader.load();
            FinController finController = fxmlLoader.getController() ;
            finController.setP(p);
            finController.fin();
           /* FinController finController = fxmlLoader.getController() ;
            finController.setP(p);*/
            Stage stage = new Stage();
...
/*ImageView imgv = new ImageView() ;
        ...
        if (Test(valjoeur) ) {
           ...
           // System.out.println("color :"+Plateau[valjoeur].getColor()=="Pink" ) ;
            if(n!=null)
            {
               ...
               // imgv.setVisible(false);
                b.setContentDisplay(ContentDisplay.CENTER);
            System.out.println("avant test color");
                if(Plateau[valjoeur].getColor().equals("blue"))
                {
    ...
    public void lancer_De(ActionEvent event) {
        ...
        Joeur joeur = p.getJoeur();
        int val = De.Som();
        Info.setText("move forward " + val + " steps");
        p.setLastmsg("move forward " + val + " steps");
       ...
       /* if (num_case_act + val > 99) {
        ...
        if (num_case_actuel + val > 99) {
            ...
            if (val > 0) {
                ...
                Info.setText("go back " + val + " steps");
                p.setLastmsg("go back " + val + " steps") ;
            }
...
*/
    ...
    void demare (Partie partie)
    {
       ...
       // Start.setGraphic(i);*/
        ...
        this.p = partie  ;
        playername.setText(p.getJoeur().getUser().get_Nom());
        // this.propsP=Partie ;
